	...
	_mapDescriptions(10, "")
{
	...
	_mapDescriptions[VEGETATION] = "Vegetation Type for year %d of rep %d.  Value Index: "+ToS((int)gNoVegID)+"=NoVeg, "+(gTundraID == NODATA_BYTE ? "" : ToS((int)gTundraID)+"=Tundra, ")+ToS((int)gBSpruceID)+"=Black Spruce, "+ToS((int)gWSpruceID)+"=White Spruce, "+ToS((int)gDecidID)+"=Deciduous"+(gShrubTundraID == NODATA_BYTE ? "" : ", "+ToS((int)gShrubTundraID)+"=Shrub Tundra")+(gGraminoidTundraID == NODATA_BYTE ? "" : ", "+ToS((int)gGraminoidTundraID)+"=Graminoid Tundra")+(gWetlandTundraID == NODATA_BYTE ? "" : ", "+ToS((int)gWetlandTundraID)+"=Wetland Tundra")+(gBarrenLichenMossID == NODATA_BYTE ? "" : ", "+ToS((int)gBarrenLichenMossID)+"=Barren/Lichen/Moss")+(gTemperateRainforestID == NODATA_BYTE ? "" : ", "+ToS((int)gTemperateRainforestID)+"=Temperate Rainforest")+(gGrasslandID == NODATA_BYTE ? "" : ", "+ToS((int)gGrasslandID)+"=Grassland (grassland is in alpha)");
	...
	if (gTemperateRainforestID != RasterIO::NODATA_BYTE)
	{
		c.c1=43; c.c2=212; c.c3=224; c.c4=255; 
		_pVegColorTable->SetColorEntry(gTemperateRainforestID, &c);
	}
	// Barren-Lichen-Moss
	if (gBarrenLichenMossID != RasterIO::NODATA_BYTE)
	{
