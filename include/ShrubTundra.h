...
private:
	...
	static float			_ignitionDepressor;		//Dampens the ignition factor for any cells of type ShrubTundra.
...
public:
							...
							ShrubTundra(const Frame& rFrame, const int treeDensity=-1);
	virtual 				~ShrubTundra();
...
private:
	void					_ShrubTundra(const int treeDensity);
...
inline float                ShrubTundra::getFireProb(const Landscape* pLandscape) 
...
inline const float			ShrubTundra::getIgnitionDepressor()
{
	return _ignitionDepressor;
}

inline const float			ShrubTundra::getHumanIgnitionProb() const 
{
    return _humanIgnitionsProb; 
}

inline const unsigned char  ShrubTundra::type () const 
{ 
    return gShrubTundraID; 
}
